...
public class Generation : MonoBehaviour
{
    ...
    void InitGeneration()
    {
        for (int i = 0; i < nbAgentToGenerate; i++)
        {
            ...
            newAgent.go.transform.position = RandomNavmeshLocation(radius);
    ...
    void shoot()
    {
        ...
        if (timer >= timerMax)
        {
            ...
            {
                GameObject balle = Instantiate(bullet,allAgents[i].go.transform.position,Quaternion.identity);
				balle.GetComponent<Rigidbody>().AddForce((GameObject.Find("Player").transform.position - allAgents[i].go.transform.position).normalized * 5, ForceMode.VelocityChange);
            }
            timer = 0;
        }
    }
